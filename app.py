...
def build_marinetraffic_map(df, lat_col, lon_col, popup_fields, name_col, color_col=None, key_col=None):
    ...
    The view auto-fits to the actual spread of the data, so every marker starts on-screen
    instead of being cut off by a fixed hub-level zoom."""
    valid = df.dropna(subset=[lat_col, lon_col])

    if not valid.empty:
        min_lat, max_lat = valid[lat_col].min(), valid[lat_col].max()
        min_lon, max_lon = valid[lon_col].min(), valid[lon_col].max()
        center = [(min_lat + max_lat) / 2, (min_lon + max_lon) / 2]
    else:
        view = HUB_CENTERS.get(hub, HUB_CENTERS["Global"])
        center = view["center"]

    m = folium.Map(location=center, tiles="CartoDB positron", control_scale=True)
    ...
    if not valid.empty:
        if min_lat == max_lat and min_lon == max_lon:
            # Single unique point: fit_bounds needs a real box, so pad it manually.
            m.fit_bounds([[min_lat - 0.5, min_lon - 0.5], [max_lat + 0.5, max_lon + 0.5]])
        else:
            # Small padding so edge markers aren't flush against the map border.
            pad_lat = max((max_lat - min_lat) * 0.1, 0.05)
            pad_lon = max((max_lon - min_lon) * 0.1, 0.05)
...
# ----------------------------------------------------------------------------
    ...
    if vessels.empty or ais.empty:
        ...
        if vessel_names:
            ...
            with m7:
                kpi_card("Route", f"{origin_name} → {destination_name}")

            # ---- Live multi-factor Scope 3 calculation for this vessel ----
            st.markdown("##### 🧮 Live Emissions Calculation")
            with st.spinner("Calculating emissions from current speed, weight, distance and fuel factors..."):
                factors_v = active.get("emission_factors", pd.DataFrame())
                ports_v = active.get("ports", pd.DataFrame())

                # 1) Distance: real great-circle distance between current route ports, if known
                distance_km = None
                if not ports_v.empty and {"port_id", "latitude", "longitude"}.issubset(ports_v.columns):
                    o = ports_v[ports_v["port_id"] == row.get("home_port_id")]
                    d = ports_v[ports_v["port_id"] == row.get("destination_port_id")]
                    if not o.empty and not d.empty:
                        distance_km = haversine_km(o.iloc[0]["latitude"], o.iloc[0]["longitude"],
                                                    d.iloc[0]["latitude"], d.iloc[0]["longitude"])
                if not distance_km or distance_km <= 0:
                    distance_km = 500.0  # nominal fallback leg length

                # 2) Weight: deadweight tonnage at an assumed 70% load factor (typical utilization)
                deadweight = float(row.get("deadweight_tonnage", 0) or 0)
                load_factor = 0.70
                effective_weight_t = deadweight * load_factor

                # 3) Emission factor: match the vessel's fuel type to the closest factor row;
                #    fall back to the vessel's own carbon-intensity rating (converted to kg/tonne-km)
                fuel_type = str(row.get("fuel_type", ""))
                factor_value = None
                factor_label = None
                if not factors_v.empty and "factor_name" in factors_v.columns:
                    match = factors_v[factors_v["factor_name"].str.contains(fuel_type, case=False, na=False)]
                    if not match.empty:
                        factor_value = float(match.iloc[0]["factor_value"])
                        factor_label = match.iloc[0]["factor_name"]
                if factor_value is None and pd.notna(row.get("carbon_intensity_gco2_tnm")):
                    # gCO2/tonne-nautical-mile -> kg CO2/tonne-km (1 nm = 1.852 km)
                    factor_value = float(row["carbon_intensity_gco2_tnm"]) / 1.852 / 1000
                    factor_label = f"Vessel-specific intensity ({row['carbon_intensity_gco2_tnm']} gCO2/tonne-nm)"
                if factor_value is None:
                    factor_value = 0.01
                    factor_label = "Generic fallback factor"

                # 4) Speed-efficiency multiplier: fuel burn roughly scales with speed^3 vs. the
                #    vessel's average/design speed, so running faster than average costs more CO2e
                current_speed = float(row.get("speed_knots", 0) or 0)
                design_speed = float(row.get("average_speed_knots", current_speed) or current_speed or 1)
                speed_ratio = (current_speed / design_speed) if design_speed > 0 else 1.0
                speed_ratio = max(0.5, min(speed_ratio, 1.6))  # keep to a sane physical range
                speed_multiplier = speed_ratio ** 3

                tonne_km = effective_weight_t * distance_km
                base_co2e_kg = tonne_km * factor_value
                total_co2e_kg = base_co2e_kg * speed_multiplier
                total_co2e_t = total_co2e_kg / 1000

            calc1, calc2, calc3, calc4 = st.columns(4)
            with calc1:
                kpi_card("Distance Used", f"{distance_km:,.0f} km", "great-circle, home→destination port")
            with calc2:
                kpi_card("Effective Weight", f"{effective_weight_t:,.0f} t", f"{deadweight:,.0f} t DWT @ {load_factor:.0%} load")
            with calc3:
                kpi_card("Speed Factor", f"{speed_multiplier:.2f}×", f"{current_speed:.1f} kn vs. {design_speed:.1f} kn design")
            with calc4:
                kpi_card("Emission Factor", f"{factor_value:.4f} kg/t-km", factor_label)

            st.markdown(
                f"""<div class="vessel-card">
                        <div class="vessel-card-sub">Formula: Effective Weight × Distance × Emission Factor × Speed Factor</div>
                        <div class="vessel-route-row" style="font-size:1.3rem;">
                            <span>{effective_weight_t:,.0f} t</span><span>×</span>
                            <span>{distance_km:,.0f} km</span><span>×</span>
                            <span>{factor_value:.4f}</span><span>×</span>
                            <span>{speed_multiplier:.2f}</span><span>=</span>
                            <span style="color:#0284c7;">{total_co2e_t:,.2f} t CO2e</span>
                        </div>
                    </div>""",
                unsafe_allow_html=True,
            )
